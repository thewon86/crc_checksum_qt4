...
CRC16::CRC16(quint16 poly, quint16 seed)
{
    this->setPoly(poly);
    this->setSeed(seed);
}

QString CRC16::algorithmName() const
{
    return QString("CRC-16");
}

quint32 CRC16::CRCSum(unsigned char *data, int numb)
{
    bool lsb;
...
quint32 CRC16::CRCSum(unsigned char *data, int numb, quint32 poly, quint32 seed)
{
    this->setPoly(poly);
    this->setSeed(seed);
...
quint32 CRC16::getCRCSum() const
{
    return this->m_crc_resu;
}

QString CRC16::getCRCSumStr(quint32 crc_sum) const
{
    QString crc_s;
...
void CRC16::setPoly(quint32 poly)
{
    this->m_crc_poly = poly & 0xFFFF;
}

void CRC16::setSeed(quint32 seed)
{
    this->m_crc_seed = seed & 0xFFFF;
}
 ...
 */
quint32 CRC16::CRCTable(quint32 crc, unsigned char *data, int numb)
{
    while (numb--)
...
quint32 CRC16::CRCByte(quint32 crc, unsigned char data)
{
    return (crc >> 8) ^ ::crc16_table[(crc ^ data) & 0xff];
}

void CRC16::printCRCTable()
{
    int i;
     QFile table_f;
     QTextStream table;
     ...
     poly_s = getPolyStr(m_crc_poly, 16);
